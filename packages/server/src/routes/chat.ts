...
type BuildmindUIMessage = UIMessage<ChatMessageMetadata, never, InferUITools<ToolContracts>>

const submitSchema = z.object({
  id: z.string(),
  messages: z
    .array(
      z.custom<BuildmindUIMessage>((value) => {
...
function hasPendingToolCalls(message: BuildmindUIMessage) {
  ...
  .post(
    ...
    async (c) => {
      ...
      const previousMessages = Array.isArray(session.messages)
        ? (session.messages as unknown as BuildmindUIMessage[])
        : []
      ...
      const nextMessages = await validateUIMessages<BuildmindUIMessage>({
      ...
      return result.toUIMessageStreamResponse<BuildmindUIMessage>({
